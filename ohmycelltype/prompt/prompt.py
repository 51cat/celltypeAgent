...
# 注释流程
请按照以下步骤进行推理：
Step 1 — 细胞谱系判定：根据 canonical marker genes 判断该 cluster 所属的确定细胞大类，并说明支持该判断的关键 marker。
Step 2 — 亚型精细鉴定：在确定细胞大类后，根据更具特异性的 marker 基因进一步推断可能的细胞亚型，并说明支持依据。
## 异常情况识别
在注释过程中，请同时评估以下可能情况：
...
AUDIT_PROMPT = """
你是一位国际顶尖的单细胞转录组学专家，长期从事 {species} 细胞图谱研究。
...
1. 这些基因是否支持该细胞类型
2. marker 是否具有合理特异性
3. **重要**: 必须考虑该细胞类型是否合理存在于 {tissue}，该细胞类型是否违背已发表的 {tissue} 单细胞图谱常识
--------------------------------------------------
...
------------------------------------------------
【整合原则】
1. 不重新预测细胞类型: 你的任务仅是 **整合已有模型结果并形成共识**。
------------------------------------------------
2. 主谱系一致性
